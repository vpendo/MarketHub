...
class RegisterAPIView(APIView):
    ...
    def post(self, request):
        ...
        return Response({"user": user_to_dict(user)}, status=status.HTTP_201_CREATED)
...
class LoginAPIView(APIView):
    ...
    @extend_schema(
        ...
        description="Login with email or username and password, returns JWT tokens."
    )
    def post(self, request):
        serializer = LoginSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        email_or_username = data['email']  # can be email or username
